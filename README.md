...
1. 把整个 `bone_weight_editor` 文件夹复制到目标项目根目录的 `addons/` 下：
   `<目标项目>/addons/bone_weight_editor/`（须包含 `plugin.cfg`、`.gd`、`.tscn`、各自 `.uid`）。
2. 启用插件（二选一）：
   - 编辑器界面：**Project → Project Settings → 插件 / Plugins** → 勾选
     **骨骼权重编辑器 (Bone Weight Editor)**。
   - 或直接在该项目的 `project.godot` 里加：
     ```
     [editor_plugins]
